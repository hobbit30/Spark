...
class Sparkchain:
    ...
    def welcome(self):
        ...
        {Fore.WHITE + Style.BRIGHT}
            ...
            """
            f"""
        {Fore.YELLOW + Style.BRIGHT}🤖Sparkchain - BOT {Fore.YELLOW + Style.BRIGHT}| Run Multi Node & Multi Accounts💰
            """
        )
    ...
    async def load_proxies(self, use_proxy_choice: int):
        ...
        try:
            ...
            if not self.proxies:
                self.log(f"{Fore.RED + Style.BRIGHT}No Proxies Found.{Style.RESET_ALL} ⚠️")
                return
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Fore.CYAN + Style.BRIGHT}[{datetime.now().astimezone(wib).strftime('%x %X %Z')}]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.RED + Style.BRIGHT}[ 🛑 EXIT ] Sparkchain - BOT 🤖{Style.RESET_ALL}                                       "                              
        )